...
| 文档 | Markview | SuperGoodViewer | MarkText |
|:--|--:|--:|--:|
| 10 KiB 正文 | 0.12 | 0.61 | 1.13 |
| 100 KiB 正文 | 0.12 | 0.94 | 1.12 |
| 10 KiB，108 个行间公式 | 0.12 | 无法渲染¹ | 1.39 |
| 100 KiB，1092 个行间公式 | 0.11 | 无法渲染¹ | 3.24 |

文档上屏后常驻内存，单位 MiB，含每个阅读器的全部进程：

| 文档 | Markview | SuperGoodViewer | MarkText |
|:--|--:|--:|--:|
| 10 KiB 正文 | 46 | 296 | 696 |
| 100 KiB 正文 | 46 | 347 | 706 |
| 10 KiB，108 个行间公式 | 48 | — | 753 |
| 100 KiB，1092 个行间公式 | 50 | — | 1143 |

¹ SuperGoodViewer 的 LaTeX 转换不接受该样本里的矩阵（`unknown variable:pmatrix`），窗口停在编译错误提示上，因此记录为无法渲染而不是计时。
...
- **滚动可以平滑。** 在**设置**中开启“Smooth scrolling”，或设置 `settings.toml` 中的 `smooth-scroll`，即可让 `Page Up`/`Page Down`、`Space`、`Home`/`End`、方向键、滚轮、点击滚动条轨道以及 `#标题锚点` 跳转在 120–400 ms 内缓动；滚轮反向转动时从当前位置接管，而不是先走完尚未完成的滚动。拖动滑块以及其他滚动保持即时。默认关闭。
